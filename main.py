...
class Square(p.sprite.Sprite):
    def __init__(self, x_id, y_id, number):
        ...
        # to make sure width and height is correct
...
# Function to handle checking for dangerous positions
def checkDangerPos():
...
# Function to check for available edge positions
def checkEdge():
    global compMove, move
...
# Function to get the positions of winning line
def getPos(n1, n2):
...
# Define dangerous positions for AI
dangerPos1 = ['', 'x', '', '', '', 'o', '', '', '', 'x']
dangerPos2 = ['', '', '', 'x', '', 'o', '', 'x', '', '']
dangerPos3 = ['', '', '', 'x', 'x', 'o', '', '', '', '']
dangerPos4 = ['', 'x', '', '', '', 'o', 'x', '', '', '']
...
# Define positions for drawing winning line
startX = 0
startY = 0
endX = 0
endY = 0

# main loop creating squares
num = 1
for y in range(1, 4):
    for x in range(1, 4):
        sq = Square(x, y, num)
